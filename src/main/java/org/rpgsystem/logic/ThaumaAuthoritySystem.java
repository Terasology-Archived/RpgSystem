 ...
 */
package org.terasology.stamina;
...
import org.terasology.stamina.component.StaminaComponent;
...
public class StaminaSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    private static final Logger logger = LoggerFactory.getLogger(StaminaSystem.class);
    ...
    public void update(float delta) {
        ...
        for (EntityRef entity : entityManager.getEntitiesWith(StaminaComponent.class)) {
        	StaminaComponent stamina = entity.getComponent(StaminaComponent.class);

            //Check to see if health should be decreased
            if (StaminaUtils.getStaminaForEntity(entity) < stamina.healthLossThreshold) {
                if (gameTime >= stamina.nextHealthDecreaseTick) {
                    entity.send(new DoDamageEvent(stamina.healthDecreaseAmount));
                    stamina.nextHealthDecreaseTick = gameTime + stamina.healthDecreaseInterval;
                }
            }
        }
    }
